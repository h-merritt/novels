...
# can also use this code for professions
...
def s_weat(X, A, B, permt = 0, perm_n = 50, table = "era2013to2022", cohesion_test = False,
...
tablenames = ['coming_of_age', 'era1920to1945', 'era1946to1971', 'era1972to1997', 'era1998to2022']
for vector, table in zip(vectors, tablenames):
	...
	if __name__ == '__main__':
		...
		results = []
		with open("professions_12.14.22.txt", 'r') as f:
			for line in f:
				attribute = str(line.rstrip())
				try:
					result_dict = s_weat([attribute], vecs["male"], vecs["female"],
							permt=2, perm_n=50, table = table, cohesion_test = True, cohesion_permutations = 50, cohesion_type = 1)
		...
		dfname = "embeddings_professions_" + table + ".csv"
		df.to_csv(dfname, index = False)

		results = []
		with open("work_list_12.15.22.rtf", 'r') as f:
			for line in f:
				attribute = str(line.rstrip())
				try:
					result_dict = s_weat([attribute], vecs["male"], vecs["female"],
		...
		df = pd.DataFrame.from_dict(results)
		dfname = "embeddings_work_" + table + ".csv"
		df.to_csv(dfname, index = False)
