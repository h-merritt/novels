...
def group_cohesion_test(X, Y, perm_n = 50, permtype = 1):
...
def weat(X, Y, A, B, permt = 0, perm_n = 50, table = "coming_of_age", cohesion_test = False,
cohesion_permutations = 50, cohesion_type = 2):
...
def s_weat(X, A, B, permt = 0, perm_n = 50, table = "coming_of_age", cohesion_test = False,
cohesion_permutations = 50, cohesion_type = 2):
...
tablenames = ['coming_of_age', 'era1920to1945', 'era1951to1981', 'era1982to2012', 'era2013to2022']
vectors = ['vectors.db', 'vectors_1920to1950.db', 'vectors_1951to1981.db', 'vectors_1982to2012.db', 'vectors_2013to2022.db']

for vector,table in zip(vectors,tablenames):
	cnx = sqlite3.connect(vector)
	...
	if __name__ == '__main__':
		...
		vecs = {
			"female": "woman girl sister she her hers daughter mother girlfriend wife".split(),
			"male": "man boy brother he him his son father boyfriend husband".split(),
			...
			"home": "baby house home wedding family marry children kitchen".split(),
			"work": "work office job business trade money professional manage".split(),
			"nature": "nature dog horse hunt fish animal woods mountain forest bear ocean sea".split(),
			"communion": "caring generous warm helpful affectionate loving reliable tender gentle".split(),
			"agency": "persistent capable independent confident determined daring bold smart tough".split(),
			...
			"good": "good happiness happy fun fantastic magical delight joy honest laughter love".split(),
			"bad": "bad torture murder wreck die disaster killer stress kill death fight".split(),
		}
		...
		try:
			for (att1, att2, att3) in s_tests:
				result_dict = s_weat(vecs[att3], vecs[att1], vecs[att2],
						permt=2, perm_n=50, table = table, cohesion_test = True, cohesion_permutations = 100, cohesion_type = 1)
				result_dict["categories"] = att1 + " vs. " + att2
				result_dict["attributes"] = att3
			...
			for (att1, att2, att3, att4) in db_tests:
				result_dict = weat(vecs[att1], vecs[att2], vecs[att3], vecs[att4],
						permt=2, perm_n=50, table = table, cohesion_test = True, cohesion_permutations = 100, cohesion_type = 1)
				result_dict["categories"] = att1 + " vs. " + att2
				result_dict["attributes"] = att3 + " vs. " + att4
				results.append(result_dict)
		except Exception as e:
			print(e)

		df = pd.DataFrame.from_dict(results)
		dfname = "embeddings_results_comparisons_" + table + ".csv"
